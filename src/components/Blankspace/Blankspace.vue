...
export default {
	...
	methods: {
        ...
        */		
        ...
        /*
        |---------------------------------------------------------------------------------------------------------------------------
        | Count all errors found in text (double space etc). Used in proccessTextCore_External(). (Used to be in proccessTextCore())
        |---------------------------------------------------------------------------------------------------------------------------
        |
        |
        */	
		
		//countAllFoundErrors(){}, //gone to /sub_functions/core_doubleSpace_remover_check.js -> countAllFoundErrors_External(that)
		...
		/*
        |----------------------------------------------------------------------------------------------------------------------------------------------------
        | Function to highlight Errors in text (i.e double spaces, space+comma, etc). Used in proccessTextCore_External(). (Used to be in proccessTextCore())
        |----------------------------------------------------------------------------------------------------------------------------------------------------
        |
        */
		
		//highlightErrors(){},  //gone to /sub_functions/core_doubleSpace_remover_check.js -> highlightErrors_External(that)
        ...
        */
		run_typo_js_handler(){
			...
			this.$nextTick(function () {
			    ...
			    setTimeout(function(){ 	
			        that.run_typo_js_spellCheckLibrary();  //delay is a lame Fix for correct loader appearance(otherwise it appears only when {run_typo_js_spellCheckLibrary()} is finished)
			    }, 100); 
			...
			/*
			//Promise -> Var 2 //Working, but has to setTimeout 120 vs 90 in Var 1 above (for correct loader appearance) ????
			new Promise(function(resolveX, rejectMe) {
				that.typoJSCheckFlag = true; //CSS flag for button text and Loader appearance
                setTimeout(function(){ 	resolveX(); }, 120); //resolve the promise //rejectMe("rejectMe was triggered");  //delay is a lame Fix for correct loader appearance(otherwise it appears only when {run_typo_js_spellCheckLibrary()} is finished) 				
			})
			.then(
			    responseZZ => {  
        ...
        */
        ...
        |
        */
		// checkFileExist(urlToFile) {}, //gone to /sub_functions/check_if_path_exist_function.js -> checkFileExist_External()
